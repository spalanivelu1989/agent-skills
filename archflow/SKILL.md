...
3. An **ArchFlow demo** — an animated, playable visualization of the same request traveling through every component, built from the workflow diagram's phases and messages, as a React component (`*.tsx` + `*.css`) and a zero-dependency `index.html` that opens directly in a browser. The engine ships with play/pause, ⏮ back / ⏭ step (one step at a time), a draggable timeline scrubber, a clickable activity log (every entry jumps to that step), a click-to-open node inspector showing each component's description, connections, and step appearances, draggable node cards (links stay attached and re-route live — lets the viewer pull cards apart when edges overlap; positions persist across reloads via localStorage, and a ⇱ Layout button restores the authored layout), and removable connections (click any link to open a connection inspector listing what it carries and how many later steps cutting it would strand, then ✂ Remove it — the link is drawn severed, steps over it are **blocked**, steps downstream of the break are **skipped as unreachable**, the run still finishes with a "2 blocked, 4 never reached" summary, and a ⛓ Links button reconnects everything; removals persist across reloads too).
...
No — it keeps running, but the break propagates, which is the honest middle ground between "carry on as if nothing happened" (overstates resilience) and "halt at the first cut" (overstates fragility, and is wrong for optional side-paths). `computeStatuses()` walks STEPS once, keeping a `reached` set:

- **`blocked`** — the step's own link was cut. It doesn't animate; the initiator flashes red with a "✕ No connection to X" bubble and the log entry goes red. Its destination does **not** join `reached`.
- **`unreachable`** — nothing ever reached the step's initiator, because something upstream was blocked. Nothing is drawn at all; the log entry goes grey with "skipped — never reached".
- **`ok`** — runs normally, and both ends join `reached`.

Neither blocked nor unreachable steps get done-marks, and the completion line reports the damage ("Run completed — 2 blocked, 4 never reached"). Two details matter if you touch this: (1) the initiator of a `roundTrip` step is `t`, not `f` (the engine's asker/responder convention), so `stepInitiator()` must be used everywhere rather than plain `s.f` — otherwise every round-trip goes unreachable the moment anything is cut; (2) `reached` is seeded from `ORIGINS`, the nodes whose first appearance in STEPS is as an initiator. That's what lets a legitimately self-starting step (a nightly job firing in a late phase) keep running when an unrelated link is cut, instead of being falsely reported as stranded. The function is pure and order-independent, so an animated run and a scrubber jump always agree.
