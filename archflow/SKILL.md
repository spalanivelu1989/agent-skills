...
No. Clicking a link opens the connection inspector (what it carries, which steps use it) and ✂ Remove drops that pairKey into a `REMOVED_LINKS` set, persisted per demo under `archflow-removed-links:<TITLE>`. That one set drives everything: the link renders severed (red hairline + ✕, arrowheads hidden), and any step traveling it becomes a **blocked** step — it doesn't animate, the source node flashes red with a "no connection to X" bubble, the activity log entry turns red, and neither end gets a done-mark. The run continues past it, which is the point: "what still works if this link goes away?" Self-working steps (`f === t`) never travel a link, so they can never be blocked. Everything is derived from STEPS, so no per-node or per-step authoring is needed — but if you add a new interaction pattern to the engine, remember blocked steps must resolve their promise (see `runStep`) or the ⏭ Step button stays disabled forever.
