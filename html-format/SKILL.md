...
- Confirm the file is self-contained: no local CSS or JS files, no build step.
  The two Google Fonts `<link>`s are the only external dependency, and the font
  stacks degrade to system fonts if they fail to load.
